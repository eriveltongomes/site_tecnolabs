...
<div id="js-tk-admin-wrapper">
    ...
    <div id="js-tk-cparea">
            ...
            <div id="jsstadmin-wrapper-top-left">
                <div id="jsstadmin-breadcrunbs">
                    <ul>
                        ...
                        <li><?php echo JText::_('Announcements'); ?></li>
                    </ul>
                </div>
            </div>
        ...
        <div id="js-tk-heading">
            <h1 class="jsstadmin-head-text"><?php echo JText::_('Announcements'); ?></h1>
            <?php $link = JFilterOutput::ampReplace('index.php?option=' . $this->option . '&c=announcements&layout=formannouncement'); ?>             
        ...
        <form action="index.php" method="post" name="adminForm" id="adminForm" class="jsstadmin-data-wrp">
            <div id="js-tk-filter">
                <div class="tk-search-value"><input type="text" name="filter_title" placeholder="<?php echo JText::_('Title'); ?>" id="filter_title" value="<?php if (isset($this->lists['filter_title'])) echo $this->lists['filter_title']; ?>" class="text_area"/></div>
                ...
                <div class="tk-search-button">
                    ...
                    <button class="js-form-reset" onclick="/*this.form.getElementById('filter_type').value = '';*/document.getElementById('filter_title').value = '';document.getElementById('filter_categoryid').value = '';this.form.submit();"><?php echo JText::_('Reset'); ?></button>
                </div>
            </div>
            <?php
            if (!(empty($this->announcements)) && is_array($this->announcements)) {  ?>
                <table id="js-table" class="js-ticket-box-shadow">
                        ...
                        $i = 0;
                        $k = 0;
                        foreach ($this->announcements AS $row) {
                            $checked = JHTML::_('grid.id', $i, $row->id);
                            if($row->status == 1) $icon_status = 'good.png'; else $icon_status = 'close.png';
                            $editlink = JFilterOutput::ampReplace('index.php?option=' . $this->option .'&c=announcements&layout=formannouncement&cid[]=' . $row->id);
                            $deletelink = JFilterOutput::ampReplace('index.php?option=' . $this->option .'&c=announcements&task=deleteannouncement&cid[]=' . $row->id .'&'. JSession::getFormToken() .'=1'); ?>
                            <tr>
                                <td class="center"><?php echo $k + 1 + $this->pagination->limitstart; ?></td>
                                <td><a href="<?php echo $editlink;?>"><?php echo $row->title; ?></a></td>
                                <td><?php echo JText::_($row->anncategory); ?></td>
                                <?php /*<td><?php if ($row->type == 1) echo JText::_('Public');elseif ($row->type == 2) echo JText::_('Private'); ?></td>*/ ?>
            ...
            <input type="hidden" name="layout" value="announcements"/>
