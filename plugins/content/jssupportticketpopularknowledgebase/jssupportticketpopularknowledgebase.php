 ...
 * Created on:	Aug 25, 2010
  ^
  + Project: 		JS Jobs
 * File Name:	Pplugin/jssupportticketknowledgebase.php
  ^
...
class plgContentjssupportticketpopularknowledgebase extends JPlugin {

    /**
     * onContentPrepare is for Joomla 1.6
     */
    public function onContentPrepare($context, &$row, &$params, $page = 0) {
        if (JString::strpos($row->text, 'jssupportticketknowledgebase') === false) {
            return true;
        }

        // expression to search for
        $regex = '/{jssupportticketknowledgebase\s*.*?}/i';
    ...
    protected function _process(&$row, &$matches, $count, $regex, $style) {
        for ($i = 0; $i < $count; $i++) {
            $load = str_replace('jssupportticketknowledgebase', '', $matches[0][$i]);
    ...
    protected function _load($position, $style = -2) {
        ...
        $content = JSSupportTicketModel::getJSModelForMP('moduleplugin')->getContentForMP($title,$showtitle,$titlebackgroundcolor,$titlecolor,2,$viewall,$maxrecord,$recordperrow,$textoverflow,$itemid,'knowledgebase',null);
        return $content;
    }
