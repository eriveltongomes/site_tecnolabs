...
class plgSystemRsseo extends JPlugin
{
	...
	public function onBeforeRender() {
		...
		if (!$this->canRun() || !JFactory::getApplication()->isClient('site')) {
			return false;
		}
			...
			if ($component->id == $table->extension_id) {
				...
				if ($private['error'] == 0 && $private['size'] > 0) {
					...
					if (strtolower($extension) == 'json') {
						if (JFile::upload($private['tmp_name'], JPATH_ADMINISTRATOR.'/components/com_rsseo/assets/keys/'.md5($secret.'private_key').'.json')) {
							$app->enqueueMessage(JText::_('COM_RSSEO_GSA_KEY_UPLOADED'));
						} else {
							$app->enqueueMessage(JText::_('COM_RSSEO_GSA_KEY_UPLOADED_ERROR'), 'error');
						}
					} else {
						$app->enqueueMessage(JText::_('COM_RSSEO_GSA_KEY_WRONG_EXTENSION'), 'error');
					}
				}
			}
		}
	...
	protected function redirect() {
		...
		if (!JFactory::getConfig()->get('sef') || $crawl || !$app->isClient('site')) {
			return;
		}
	...
	protected function removeCSSJs(&$content, &$change) {
		if (!JFactory::getApplication()->isClient('site') || JFactory::getDocument()->getType() != 'html') {
			return false;
		}
	...
	protected function optimize(&$content, &$change) {
		...
		if (!JFactory::getApplication()->isClient('site') || JDEBUG || JFactory::getDocument()->getType() != 'html') {
			return false;
		}
